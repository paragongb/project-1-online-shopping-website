...
/**
 * REST controller for managing {@link com.paragon.project1.domain.Address}.
 */
@RestController
...
public class AddressResource {
    ...
    public List<AddressDTO> getAllAddresses(@RequestParam(name = "filter", required = false) String filter) {
        if ("shippingorder-is-null".equals(filter)) {
            LOG.debug("REST request to get all Addresss where shippingOrder is null");
            return addressService.findAllWhereShippingOrderIsNull();
        }

        if ("billingorder-is-null".equals(filter)) {
            LOG.debug("REST request to get all Addresss where billingOrder is null");
            return addressService.findAllWhereBillingOrderIsNull();
        }
