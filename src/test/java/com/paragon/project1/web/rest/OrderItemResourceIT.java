...
import static com.paragon.project1.domain.OrderItemAsserts.*;
...
import com.paragon.project1.service.OrderItemService;
...
class OrderItemResourceIT {
    ...
    private static final BigDecimal DEFAULT_PRICE_AT_PURCHASE = new BigDecimal(0);
    private static final BigDecimal UPDATED_PRICE_AT_PURCHASE = new BigDecimal(1);
    ...
    @Test
    @Transactional
    void checkPriceAtPurchaseIsRequired() throws Exception {
        long databaseSizeBeforeTest = getRepositoryCount();
        // set the field null
    ...
    void getAllOrderItemsWithEagerRelationshipsIsEnabled() throws Exception {
        when(orderItemServiceMock.findAllWithEagerRelationships()).thenReturn(new ArrayList<>());

        restOrderItemMockMvc.perform(get(ENTITY_API_URL + "?eagerload=true")).andExpect(status().isOk());

        verify(orderItemServiceMock, times(1)).findAllWithEagerRelationships();
    }
